...
const RegisterService = async (payload) => {
    ...
    const ExistingUser = await db_1.prisma.customer.findUnique({
        where: {
            email,
        },
    });
    ...
    const user = await db_1.prisma.customer.create({
        data: {
            name,
            email,
            password: hashedPass,
...
const LoginService = async (payload) => {
    ...
    const user = await db_1.prisma.customer.findUniqueOrThrow({
        where: {
            email,
        },
    });
